...
func promptValue(attr string) float64 {
  scanner := bufio.NewScanner(os.Stdin)
  fmt.Printf("Enter value for %s: ", attr)
  scanner.Scan()
  value := scanner.Text()
...
func main() {
  acceleration := promptValue("acceleration")
  velocity := promptValue("velocity")
  displacement := promptValue("displacement")
  time := promptValue("time")
